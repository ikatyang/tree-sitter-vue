...
static inline TagType get_tag_from_string(const char *tag_name) {
    for (int i = 0; i < 126; i++) {
...
static inline Tag make_tag(TagType type, const char *name) {
    Tag tag = new_tag();
    tag.type = type;
    if (type == CUSTOM) {
...
static inline bool can_contain(Tag *self, const Tag *other) {
